...
raw_ped <- read_table("/Users/aja294/Documents/Trout_local/trout-app/Data/even_year_ped.txt") %>%
...
candidates <- read_table("/Users/aja294/Documents/Trout_local/trout-app/Data/2022_candidates_0924.txt")
...
df <- as_tibble(selected_matrix, rownames = "Row") %>%
  pivot_longer(-Row, names_to = "Female", values_to = "Kinship") %>%
  select(Female, Row, Kinship) %>%
  rename(Male = Row)
  
# Add a column with the quantile names
